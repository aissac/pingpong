...
//! CRITICAL FIX: Token pair mapping instead of broken XOR trick
...
/// Run the synchronous hot path with token pair mapping
pub fn run_sync_hot_path(
    tx: Sender<BackgroundTask>,
    tokens: Vec<String>,
    killswitch: Arc<AtomicBool>,
    token_pairs: HashMap<u64, u64>,  // YES hash -> NO hash mapping
) {
    ...
    if let Some(asset_idx) = asset_pattern.find(bytes) {
        ...
        if token_start + 66 <= len {
            ...
            if let Some(price_idx) = price_pattern.find(bytes) {
                let price_val_start = price_idx + 9;
                                ...
                                if size_start < len {
                                    if let Some(size_end) = memchr(b'"', &bytes[size_start..]) {
                                        ...
                                        if size_end_idx <= len {
                                            ...
                                            // DEBUG: Log orderbook every 100 updates
                                            static UPDATE_COUNTER: AtomicU64 = AtomicU64::new(0);
                                            let uc = UPDATE_COUNTER.fetch_add(1, Ordering::Relaxed);
                                            if uc % 100 == 0 {
                                                println!("[DEBUG] Orderbook size: {}", orderbook.len());
                                            }
                                            
                                            // 5. Edge detection - USE TOKEN PAIR MAPPING
