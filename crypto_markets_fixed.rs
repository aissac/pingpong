...
/// Market info extracted from Gamma API
#[derive(Debug, Clone)]
...
pub async fn fetch_active_crypto_markets(
    ...
    let resp = match client.get(url).send().await {
        ...
        Err(e) => {
            ...
            return (token_ids, token_pairs, token_strings);
        }
    };
            ...
            for market in markets {
                ...
                if let Some(tokens) = market["tokens"].as_array() {
                    if tokens.len() == 2 {
                        ...
                        token_pairs.insert(yes_token.clone(), no_token.clone());
                        token_strings.push(event_slug.to_string());
                    }
                }
            }
        }
    ...
    (token_ids, token_pairs, token_strings)
}
