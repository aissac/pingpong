...
//! - Background Telegram reporter (isolated thread)
...
use pingpong::telegram_reporter::{PnLStats, start_telegram_reporter};
...
/// Telegram config
const TELEGRAM_CHAT_ID: &str = "1798631768";
...
fn main() {
    ...
    // PnL stats (shared between hot path and Telegram reporter)
    let pnl_stats = Arc::new(PnLStats::new());
    
    // Start Telegram reporter background thread
    let telegram_token = std::fs::read_to_string("/home/ubuntu/telegram_bot_token")
        .expect("Failed to read telegram_bot_token")
        .trim()
        .to_string();
    
    let _telegram_handle = start_telegram_reporter(
        Arc::clone(&pnl_stats),
        telegram_token,
        TELEGRAM_CHAT_ID.to_string(),
        360, // Report every 6 minutes
    );
    info!("📊 Telegram reporter started (6-minute intervals)");
