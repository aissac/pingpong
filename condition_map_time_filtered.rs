...
//! - startDate <= now <= endDate (TIME-BASED FILTERING)
//! 
...
pub async fn build_maps(
    ...
    println!("📊 Querying Gamma API for ACTIVE binary markets (time-filtered)...");
            ...
            match resp.json::<Value>().await {
                Ok(json) => {
                    if let Some(events) = json.as_array() {
                        ...
                        for event in events {
                            // Check event-level dates first
                            let event_start = event["startDate"].as_str()
                            ...
                            if let Some(markets) = event["markets"].as_array() {
                                for market in markets {
                                    // Check market-level dates
                                    let market_start = market["startDate"].as_str()
                                        .and_then(parse_iso_date);
                                    ...
                                    // Skip not-yet-started markets (optional: pre-subscribe)
                                    if let Some(start) = market_start {
                                        if start > now {
                                            skipped_not_started += 1;
                                            continue; // Skip future markets
                                        }
                                    }
                        ...
                        println!("✅ ACTIVE markets: {} (expired: {}, not-started: {})", 
                            market_count, skipped_expired, skipped_not_started);
                    }
                }
