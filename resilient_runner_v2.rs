...
use crate::hft_hot_path::run_sync_hot_path;
...
pub fn start_resilient_bot() {
    ...
    // Create localized Tokio runtime for async Gamma API fetches
    ...
    loop {
        ...
        // Fetch current active markets from Gamma API (async wrapped in block_on)
        let all_tokens: Vec<String> = rt.block_on(async {
            // Placeholder - will be populated by actual fetch
            // For now, return empty and let rollover populate
            Vec::new()
        });
        ...
        // STEP 2: 🚨 SET TCP READ TIMEOUT (30 seconds) 🚨
        ...
        // STEP 3: TLS + WebSocket Handshake
        println!("🔐 Performing TLS/WS handshake...");
        let request = url.clone().into_client_request().unwrap();
        let mut ws_stream = match tungstenite::client_tls(request, tcp_stream) {
            Ok((stream, _response)) => {
        ...
        // STEP 4: Create channels for this connection
        let (rollover_tx, rollover_rx) = bounded::<crate::hft_hot_path::RolloverCommand>(64);
        let (background_tx, _) = bounded::<crate::hft_hot_path::BackgroundTask>(1024);
        let (log_tx, log_rx) = bounded::<crate::jsonl_logger::LogEvent>(4096);
        let _logger_handle = JsonlLogger::start(log_rx);
        
        // STEP 5: Start rollover thread
        ...
        // STEP 6: Build bi-directional token pairs
        ...
        // This blocks until timeout (30s silence) or error triggers return
