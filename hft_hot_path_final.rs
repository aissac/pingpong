//! HFT Hot Path - Fixed debug output for Combined ASK
...
pub fn run_sync_hot_path<R: Read>(
    ...
    let mut combined_print_count = 0u64;
    
    println!("[HFT] 🔥 Starting hot path with Combined ASK debug...");
                    ...
                    if let Some(price_end) = memchr(b'"', &bytes[price_val_start..]) {
                        ...
                        if let Some(size_idx) = size_pattern.find(&bytes[size_search_start..]) {
                            ...
                            if let Some(size_end) = memchr(b'"', &bytes[size_start..]) {
                                ...
                                // Check for edge after every update
                                if let Some(&complement_hash) = token_pairs.get(&token_hash) {
                                    if let (Some(yes_state), Some(no_state)) = 
                                        ...
                                        // BOTH must have bid AND ask to calculate Combined ASK
                                        if let (Some((yes_bid_price, _yes_bid_size)), Some((yes_ask_price, yes_ask_size))) = 
                                            (yes_state.get_best_bid(), yes_state.get_best_ask()) {
                                            if let (Some((no_bid_price, _no_bid_size)), Some((no_ask_price, no_ask_size))) = 
                                                (no_state.get_best_bid(), no_state.get_best_ask()) {
                                                
                                                // Combined ASK in cents (price_cents is 0-99)
                                                let combined_ask_cents = yes_ask_price + no_ask_price;
                                                
                                                // Print Combined ASK (limit to first 100)
                                                if combined_print_count < 100 {
                                                    combined_print_count += 1;
                                                    println!("[COMBINED] YES: Ask=${:.2} Bid=${:.2} | NO: Ask=${:.2} Bid=${:.2} => Combined=${:.2}{}",
                                                        yes_ask_price as f64 / 100.0,
                                                        yes_bid_price as f64 / 100.0,
                                                        no_ask_price as f64 / 100.0,
                                                        no_bid_price as f64 / 100.0,
                                                        combined_ask_cents as f64 / 100.0,
                                                        if combined_ask_cents < 98 { " *** EDGE ***" } else { "" }
                                                    );
                                                }
                                                
                                                // Check for edge (< 98 cents)
                                                if combined_ask_cents < 98 && combined_ask_cents > 90 {
                                                    let ec = edge_counter.fetch_add(1, Ordering::Relaxed);
                                                    
                                                    println!("[EDGE] 🎯 FOUND! Combined ASK = ${:.4}", 
                                                        combined_ask_cents as f64 / 100.0);
                                                    println!("  YES Ask: ${:.2} (size: {}) | Bid: ${:.2}", 
                                                        yes_ask_price as f64 / 100.0,
                                                        yes_ask_size,
                                                        yes_bid_price as f64 / 100.0);
                                                    println!("  NO  Ask: ${:.2} (size: {}) | Bid: ${:.2}", 
                                                        no_ask_price as f64 / 100.0,
                                                        no_ask_size,
                                                        no_bid_price as f64 / 100.0);
                                                }
