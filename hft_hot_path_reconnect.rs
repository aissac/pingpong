//! HFT Hot Path - With WebSocket Reconnection
//!
//! Automatically reconnects when WebSocket closes.
...
/// Run the hot path with automatic reconnection.
/// 
/// IMPORTANT: On reconnect, orderbook state is CLEARED.
/// This prevents stale data from causing false edge detections.
pub fn run_sync_hot_path<R: Read>(
    ...
    // Orderbook state - will be cleared on reconnect
    ...
    println!("[HFT] 🔥 Starting hot path with reconnection support...");
    ...
    loop {
        ...
        let n = match ws_stream.read(&mut buffer[total_bytes..]) {
            Ok(0) => {
                ...
                // CLEAR ORDERBOOK STATE - CRITICAL
                // Prevent stale data from causing false edges
                orderbook.clear();
                for token in &all_tokens {
                    orderbook.entry(fast_hash(token.as_bytes()))
                        .or_insert_with(TokenBookState::new);
            ...
            Err(e) => {
                reconnect_count += 1;
                println!("[HFT] ⚠️ WebSocket error: {}. Reconnect #{}", e, reconnect_count);
        ...
        if messages == 50 && !debug_printed {
            ...
            if reconnect_count > 0 {
                println!("[HFT] Reconnects: {}", reconnect_count);
            }
        }
    }
