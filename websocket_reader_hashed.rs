...
//! 
//! FIX (2026-04-01): Hash token IDs before subscribing to WebSocket
//! Old code used hashed u64 IDs, we were sending raw 77-char strings
...
pub fn connect_to_polymarket(tokens: Vec<String>) -> WebSocketReader {
    ...
    // FIX: Hash token IDs to u64 before subscribing (matches old code behavior)
    let tokens_hashed: Vec<u64> = tokens.iter()
        .map(|t| crate::token_map::hash_token(t))
        .collect();
    
    println!("Subscribing to {} hashed tokens", tokens_hashed.len());
    
    // Subscribe with hashed token IDs
    let subscribe_msg = serde_json::json!({
        "type": "market",
        "operation": "subscribe",
        "markets": [],
        "assets_ids": tokens_hashed,
        "initial_dump": true
    });
    ...
    println!("Subscribed to {} tokens", tokens_hashed.len());
...
impl Read for WebSocketReader {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        match self.socket.read() {
            ...
            Ok(Message::Ping(_)) => {
                // Handle ping automatically (tungstenite responds with pong)
                Ok(0)
            }
            Ok(Message::Pong(_)) => {
