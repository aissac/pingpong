...
pub async fn run_user_ws(
    ...
    let (mut ws_stream, _) = connect_async(USER_WS_URL)
        .await
...
async fn handle_event(
    ...
    match event_type {
        "MATCHED" => {
            ...
            if let Some(ctx) = state.pending_hedges.get(order_id) {
        ...
        "MINED" => {
            ...
            if let Some(mut ctx) = state.pending_hedges.get_mut(order_id) {
                ...
                if let Some(mut ctx) = state.pending_hedges.get_mut(maker_id.clone()) {
