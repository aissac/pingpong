...
//! - Rollover: background thread checks every 1 minute for market transitions
...
/// Target combined price threshold ($0.98 = 980,000 micro-USDC)
...
fn main() {
    ...
    // 5. INITIALIZE SIGNER
    // ============================================================
    let signer = Arc::new(temp_rt.block_on(async {
        init_signer(&private_key).await
    }));
    ...
    let rollover_tx_thread = rollover_tx.clone();
    
    thread::spawn(move || {
        let mut state = RolloverState::new();
        
        loop {
            thread::sleep(Duration::from_secs(60));  // Check every 1 minute
            
            // Get current market periods
            let periods = get_current_periods();
            
            // TODO: Fetch actual token IDs from Gamma API for each period
            // For now, we just log the rollover check
            let now = Utc::now();
            println!("[ROLLOVER] 🔄 Checked at {:02}:{:02}:{:02} - {} periods tracked", 
                now.hour(), now.minute(), now.second(), state.active_periods.len());
            
            // Check for rollover
            let (to_subscribe, to_unsubscribe) = check_rollover(&mut state, &periods);
            
            if !to_subscribe.is_empty() {
                println!("[ROLLOVER] 📡 Would subscribe to {} tokens", to_subscribe.len());
                // TODO: Actually send subscription when token IDs are populated
                // if let Err(e) = rollover_tx_thread.send(RolloverCommand::Subscribe { tokens: to_subscribe }) {
                //     eprintln!("[ROLLOVER] ❌ Failed to send subscribe: {}", e);
                // }
            }
            
            if !to_unsubscribe.is_empty() {
                println!("[ROLLOVER] 🗑️ Would unsubscribe from {} tokens", to_unsubscribe.len());
                // TODO: Actually send unsubscription
                // if let Err(e) = rollover_tx_thread.send(RolloverCommand::Unsubscribe { tokens: to_unsubscribe }) {
                //     eprintln!("[ROLLOVER] ❌ Failed to send unsubscribe: {}", e);
                // }
            }
        }
    });
    
    println!("✅ Rollover checker thread spawned (checking every 60s)");
