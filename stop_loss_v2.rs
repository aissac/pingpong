...
use crate::execution::{submit_order, build_l2_headers, fetch_fee_rate, create_order_payload, generate_salt};
    ...
    match event_type {
        ...
        "MINED" => {
            ...
            if let Some(mut ctx) = state.pending_hedges.get_mut(order_id) {
                ...
                if let Some(mut ctx) = state.pending_hedges.get_mut(maker_id) {
