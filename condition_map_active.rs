...
//! 
//! This module ONLY tracks CURRENT active markets (no past, no distant future).
//! Past markets have no liquidity (trading halted).
//! Future markets have sparse orderbooks (pre-order only).
...
/// Get current period + next period (for pre-subscription buffer)
/// This ensures we have orderbook state ready when the next period starts
fn get_active_15m_periods() -> Vec<i64> {
    let current = get_current_15m_period();
    vec![current, current + 900]  // Current + Next (pre-subscribe)
}

fn get_active_5m_periods() -> Vec<i64> {
    let current = get_current_5m_period();
    vec![current, current + 300]  // Current + Next (pre-subscribe)
}

/// Build maps from BTC/ETH 5m/15m markets - ONLY ACTIVE PERIODS
        ...
        for event in events {
            if let Some(markets) = event["markets"].as_array() {
                for market in markets {
                    ...
                    if outcomes.len() == 2 && 
                       (outcomes[0].to_lowercase() == "yes" || outcomes[1].to_lowercase() == "yes") {
