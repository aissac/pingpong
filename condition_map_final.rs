...
//! - outcomes == ["Yes", "No"]
//! - clobTokenIds has exactly 2 elements
//! 
...
/// 
/// Returns:
/// - hash_to_id: token_hash → token_id string
/// - id_to_condition: token_id → condition_id (for CTF merges)
/// - complement_map: YES_hash → NO_hash (for hot path edge detection)
                        ...
                        for event in events {
                            if let Some(markets) = event["markets"].as_array() {
                                for market in markets {
                                    ...
                                    // Extract from clobTokenIds array
                                    if let Some(clob_ids) = market["clobTokenIds"].as_array() {
                                        if clob_ids.len() == 2 {
                                            let yes_str = clob_ids[0].as_str().unwrap_or("").to_string();
                                            let no_str = clob_ids[1].as_str().unwrap_or("").to_string();
