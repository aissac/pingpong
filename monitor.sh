...
LOG_FILE="/var/log/hft_monitor.log"
HOURLY_LOG="/var/log/hft_hourly.log"
...
while true; do
    ...
    # Check reconnects
    CURRENT_RECONNECTS=$(grep -c 'Reconnect attempt' "$BOT_OUTPUT" 2>/dev/null || echo 0)
    
    # Check TCP timeouts
    CURRENT_TIMEOUTS=$(grep -c 'TCP TIMEOUT' "$BOT_OUTPUT" 2>/dev/null || echo 0)
