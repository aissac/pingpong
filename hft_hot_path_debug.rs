//! HFT Hot Path - With Combined ASK Debug Output
...
pub fn run_sync_hot_path<R: Read>(
    ...
    let mut pair_check_printed = false;
                                ...
                                if let Some(&complement_hash) = token_pairs.get(&token_hash) {
                                    if let (Some(yes_state), Some(no_state)) = 
                                        ...
                                        if let (Some((yes_ask_price, yes_ask_size)), 
                                            ...
                                            // Debug: Print Combined ASK for first few pairs
                                            if !pair_check_printed && messages < 100 {
                                                let yes_bid = yes_state.get_best_bid().map(|(p, _)| p).unwrap_or(0);
                                                let no_bid = no_state.get_best_bid().map(|(p, _)| p).unwrap_or(0);
                                                
                                                println!("[COMBINED] YES Ask=${:.4} Bid=${:.4} | NO Ask=${:.4} Bid=${:.4} => Combined=${:.4} {}",
