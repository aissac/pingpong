...
//!
//! Added: Channel for dynamic WebSocket subscription management
//! - Receives RolloverCommand from background thread
//! - Sends subscribe/unsubscribe without disconnecting
...
pub fn run_sync_hot_path(
    ...
    loop {
        ...
        // Check for rollover commands every 1 second
                ...
                match rollover_rx.try_recv() {
                    Ok(RolloverCommand::Subscribe { tokens }) => {
                        if !tokens.is_empty() {
                            ...
                            if let Err(e) = ws_stream.send(msg) {
                                eprintln!("[ROLLOVER] ❌ Subscribe failed: {}", e);
                            } else {
                                println!("[ROLLOVER] 📡 Subscribed to {} new tokens", tokens.len());
                                // Add to orderbook
                                for token in &tokens {
                                    orderbook.entry(fast_hash(token.as_bytes()))
                                        .or_insert_with(TokenBookState::new);
                    ...
                    Ok(RolloverCommand::Unsubscribe { tokens }) => {
                        if !tokens.is_empty() {
                            ...
                            if let Err(e) = ws_stream.send(msg) {
                                ...
                                println!("[ROLLOVER] 🗑️ Unsubscribed from {} expired tokens", tokens.len());
