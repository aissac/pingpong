#!/usr/bin/env python3
"""Fix rollover to check CURRENT periods (like startup does)"""

code = r'''//! Market Rollover - Dynamic Token Pair Management
//!
//! Fetches markets for CURRENT periods (same as startup).
//! Adds any active markets that aren't already tracked.
                ...
                Ok(json) => {
                    if let Some(markets) = json.as_array() {
                        if let Some(market) = markets.first() {
                            ...
                            // Only add if active AND not closed
                            if !active || closed {
                                return None;
                            }
        ...
        for asset in &assets {
            for (period_ts, timeframe) in &periods {
                ...
                match fetch_market_tokens(&client, &slug).await {
                    ...
                    None => {
                        // Market doesn't exist, is closed, or expired
                    }
                }
            }
        }
...
print('✅ Fixed rollover to check CURRENT periods (like startup)')
