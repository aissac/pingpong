#!/usr/bin/env python3
"""Fix rollover to check FUTURE periods (markets in pre-trading)"""

code = r'''//! Market Rollover - Dynamic Token Pair Management
//!
//! Fetches markets for FUTURE periods (pre-trading phase).
//! Adds markets before they start so hot path is ready.
...
/// Get FUTURE period timestamps (for markets in pre-trading)
fn get_future_periods() -> Vec<(i64, &'static str)> {
    ...
    // Add FUTURE 5m periods (+1, +2, +3 from now)
    periods.push((ts_5m + 300, "5m"));   // Next 5m (starts in 0-5 min)
    periods.push((ts_5m + 600, "5m"));   // +2 5m (starts in 5-10 min)
    periods.push((ts_5m + 900, "5m"));   // +3 5m (starts in 10-15 min)
    ...
    // Add FUTURE 15m periods
    periods.push((ts_15m + 900, "15m"));   // Next 15m (starts in 0-15 min)
    periods.push((ts_15m + 1800, "15m"));  // +2 15m (starts in 15-30 min)
    
    // Current 1h period
    let period_1h = now.with_minute(0).unwrap().with_second(0).unwrap().with_nanosecond(0).unwrap();
    let ts_1h = period_1h.timestamp();
    
    // Add FUTURE 1h period
                ...
                Ok(json) => {
                    if let Some(markets) = json.as_array() {
                        if let Some(market) = markets.first() {
                            // For pre-trading markets, active might be false but we still want tokens
                            // Just check that tokens exist
                            if let Some(tokens) = market.get("tokens").and_then(|v| v.as_array()) {
                                if tokens.len() == 2 {
                                    ...
                                    if !yes_token.is_empty() && !no_token.is_empty() {
                                        println!("[ROLLOVER] Found tokens for: {} (YES={} NO={})", 
        ...
        for asset in &assets {
            for (period_ts, timeframe) in &periods {
                ...
                match fetch_market_tokens(&client, &slug).await {
                    ...
                    None => {
                        // Market doesn't exist yet (too far in future) or closed
                    }
                }
            }
        }
        
        println!("[ROLLOVER] Checked {} slugs, found {} markets with tokens, tracking {}", 
            assets.len() * periods.len(), found_count, tracked_markets.len());
        
        // Remove markets that are no longer in future periods (expired)
...
print('✅ Fixed rollover to check FUTURE periods (pre-trading)')
