//! Fetch active crypto up/down markets with dynamic timestamps
//! 
//! Polymarket uses slugs like: btc-updown-15m-{timestamp}
//! Where timestamp is the start of the 15-minute period
...
use crate::hft_hot_path::BackgroundTask;
...
pub fn get_current_periods() -> Vec<i64> {
    ...
    let period_start = now.with_minute((minute / 15) * 15)
        .unwrap()
        .with_second(0)
        .unwrap()
        .with_nanosecond(0)
    ...
    // Current, previous 15m, and next 15m
        ...
        for &period_ts in &periods {
            ...
            if let Some(market) = fetch_market_by_slug(client, &slug_15m, &now).await {
                if market.token_ids.len() >= 2 {
                    ...
                    println!("✅ Found {} market: {} ({} min remaining)", 
                        slug_15m, market.condition_id, 
                        (chrono::DateTime::parse_from_rfc3339(
                            serde_json::Value::String(slug_15m.clone()).as_str().unwrap_or("")
                        ).unwrap_or_default().with_timezone(&Utc) - now).num_minutes());
                }
            }
            ...
            if let Some(market) = fetch_market_by_slug(client, &slug_5m, &now).await {
                if market.token_ids.len() >= 2 {
                    ...
                    all_tokens.extend(market.token_ids);
                    println!("✅ Found {} market: {}", slug_5m, market.condition_id);
                }
            }
        }
    }
