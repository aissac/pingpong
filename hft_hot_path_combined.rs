//! HFT Hot Path - With Combined ASK Debug Output (Fixed)
//! Shows actual Combined ASK values being calculated
                            ...
                            if let Some(size_end) = memchr(b'"', &bytes[size_start..]) {
                                ...
                                if let Some(&complement_hash) = token_pairs.get(&token_hash) {
                                    if let (Some(yes_state), Some(no_state)) = 
                                        ...
                                        // Check if BOTH have bid AND ask
                                        if let (Some(yes_bid), Some(yes_ask)) = (yes_state.get_best_bid(), yes_state.get_best_ask()) {
                                            if let (Some(no_bid), Some(no_ask)) = (no_state.get_best_bid(), no_state.get_best_ask()) {
                                                // BOTH pairs complete!
                                                let combined_ask = yes_ask * 10_000 + no_ask * 10_000;
                                                
                                                // Print Combined ASK for debugging (limit to first 50)
                                                if combined_printed < 50 {
                                                    combined_printed += 1;
                                                    println!("[COMBINED] YES: Ask=${:.2} Bid=${:.2} | NO: Ask=${:.2} Bid=${:.2} => Combined=${:.4}{}",
                                                        yes_ask as f64 / 100.0,
                                                        yes_bid as f64 / 100.0,
                                                        no_ask as f64 / 100.0,
                                                        no_bid as f64 / 100.0,
                                                        combined_ask as f64 / 1_000_000.0,
                                                        if combined_ask < 980_000 { " *** EDGE ***" } else { "" }
                                                    );
                                                }
                                                ...
                                                if combined_ask <= EDGE_THRESHOLD_U64 
                                                    ...
                                                    println!("  YES Ask: ${:.4} (size: {}) | Bid: ${:.4}", 
                                                        yes_ask as f64 / 100.0,
                                                        yes_state.get_best_ask().map(|(_, s)| s).unwrap_or(0),
                                                        yes_bid * 10_000 as f64 / 1_000_000.0);
                                                    println!("  NO  Ask: ${:.4} (size: {}) | Bid: ${:.4}", 
                                                        no_ask as f64 / 100.0,
                                                        no_state.get_best_ask().map(|(_, s)| s).unwrap_or(0),
                                                        no_bid * 10_000 as f64 / 1_000_000.0);
                                                }
                                            }
                                        }
                                    }
