#!/usr/bin/env python3
"""Fix rollover to only fetch ACTIVE markets (not closed/expired)"""

code = r'''//! Market Rollover - Dynamic Token Pair Management
//!
//! Fetches new markets from Gamma API using same logic as startup.
//! Only adds markets that are ACTIVE (started, not expired, not closed).
...
/// Fetch tokens for a specific market slug - only if ACTIVE
async fn fetch_market_tokens(client: &reqwest::Client, slug: &str, now: &DateTime<Utc>) -> Option<(String, String)> {
                ...
                Ok(json) => {
                    if let Some(markets) = json.as_array() {
                        if let Some(market) = markets.first() {
                            // Check if market is ACTIVE (not closed, not expired)
                            ...
                            // Check end date
                            let is_expired = if let Some(end_str) = market.get("endDate").and_then(|v| v.as_str()) {
                            ...
                            // Only proceed if market is active, not closed, and not expired
                            if !active || closed || is_expired {
                                return None;
                            }
        ...
        for asset in &assets {
            for (period_ts, timeframe) in &periods {
                ...
                match fetch_market_tokens(&client, &slug, &now).await {
                    ...
                    None => {
                        // Market doesn't exist, is closed, or is expired
                    }
                }
            }
        }
...
print('✅ Fixed rollover to only add ACTIVE markets')
