 ...
 * Supports 5m, 15m and 1h BTC/ETH Up/Down series.
 */
@Component
@Slf4j
...
public class GabagoolMarketDiscovery {
  ...
  public void discoverMarkets() {
    ...
    private static List<String> candidateUpDown5mSlugs(String assetPrefix, Instant now) {
        ...
        // Cover the previous ~3 slots, current slot, and 1 lookahead slot.
        long from = nowSec - Duration.ofMinutes(15).toSeconds();
    ...
    private DiscoveredMarket parseEventWithFullDetails(JsonNode eventNode, String slug) {
        try {
            ...
            // Determine market type
            String marketType;
            if (eventSlug.contains("updown-5m")) {
                marketType = "updown-5m";
            } else if (eventSlug.contains("updown-15m")) {
                marketType = "updown-15m";
    ...
    private Instant parseEndTimeFromSlug(String slug, String marketType) {
        try {
            if (marketType.equals("updown-5m") || marketType.equals("updown-15m")) {
                // Slug format: btc-updown-5m-1734364800 or btc-updown-15m-1734364800
                // The last part is the epoch start timestamp.
                String[] parts = slug.split("-");
                if (parts.length >= 4) {
                    String epochStr = parts[parts.length - 1];
                    long epoch = Long.parseLong(epochStr);
                    long durationSec = marketType.equals("updown-5m") ? 300L : 900L;
                    return Instant.ofEpochSecond(epoch + durationSec);
                }
            } else {
                // 1hour markets don't have epoch in slug — rely on API endDate field.
                return null;
            }
        } catch (Exception e) {
            // ignore
