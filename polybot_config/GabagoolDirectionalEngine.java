...
public class GabagoolDirectionalEngine {
    ...
    private void evaluateMarket(GabagoolMarket market, GabagoolConfig cfg, Instant now) {
        ...
        if (cfg.directionalEnabled()) {
            evaluateDirectionalMarket(market, cfg, upBook, downBook, secondsToEnd);
            return;
        }

        // Legacy market-making mode (complete-set arbitrage)
    ...
    /**
     * Directional asymmetric sizing evaluation.
     * 
     * Core principle: Pick ONE direction, size asymmetrically.
     * - Price 0.70+: SELL (fade overpriced) at 0.3x size
     * - Price 0.55-0.70: HOLD (no edge)
     * - Price 0.45-0.55: BUY at 1.0x size
     * - Price 0.30-0.45: BUY at 2.0x size (underpriced)
     * - Price <0.30: SELL (fade extreme) at 0.3x size
     */
    private void evaluateDirectionalMarket(GabagoolMarket market, GabagoolConfig cfg,
                                            TopOfBook upBook, TopOfBook downBook, long secondsToEnd) {
        ...
        // Determine signal and sizing for each side
        // UP side
        double upPrice = upBid.doubleValue();
        double upSizeMultiplier;
        boolean upIsDirectional = false;
        boolean upShouldTrade = true;
        
        if (upPrice >= 0.70) {
            // UP is expensive (>0.70) - SELL to hedge (small size)
            upSizeMultiplier = cfg.directionalSizeLow(); // 0.3x - small hedge
        } else if (upPrice >= 0.55) {
            // UP is high (0.55-0.70) - no trade
            upShouldTrade = false;
            upSizeMultiplier = 0;
        } else if (upPrice >= 0.45) {
            // UP is fair (0.45-0.55) - minimal
            upSizeMultiplier = 0.5;
        } else if (upPrice >= 0.30) {
            // UP is cheap (0.30-0.45) - BUY directional (medium size)
            upSizeMultiplier = cfg.directionalSizeHigh(); // 3.0x
            upIsDirectional = true;
        } else {
            // UP is very cheap (<0.30) - BUY directional (high conviction)
            upSizeMultiplier = cfg.directionalSizeHigh(); // 3.0x
            upIsDirectional = true;
        }
        
        // DOWN side
        double downPrice = downBid.doubleValue();
        double downSizeMultiplier;
        boolean downIsDirectional = false;
        boolean downShouldTrade = true;
        
        if (downPrice >= 0.70) {
            // DOWN is expensive (>0.70) - SELL to hedge (small size)
            downSizeMultiplier = cfg.directionalSizeLow(); // 0.3x - small hedge
        } else if (downPrice >= 0.55) {
            // DOWN is high (0.55-0.70) - no trade
            downShouldTrade = false;
            downSizeMultiplier = 0;
        } else if (downPrice >= 0.45) {
            // DOWN is fair (0.45-0.55) - minimal
            downSizeMultiplier = 0.5;
        } else if (downPrice >= 0.30) {
            // DOWN is cheap (0.30-0.45) - BUY directional (medium size)
            downSizeMultiplier = cfg.directionalSizeHigh(); // 3.0x
            downIsDirectional = true;
        } else {
            // DOWN is very cheap (<0.30) - BUY directional (high conviction)
            downSizeMultiplier = cfg.directionalSizeHigh(); // 3.0x
            downIsDirectional = true;
        }
        
        // If no edge anywhere, skip
        if (!upShouldTrade && !downShouldTrade) {
            log.debug("DIRECTIONAL: {} - no edge (UP={}, DOWN={})", market.slug(), upPrice, downPrice);
            orderManager.cancelMarketOrders(market, CancelReason.INSUFFICIENT_EDGE, secondsToEnd);
            return;
        }
        
        // Calculate base exposure and shares
        BigDecimal exposure = quoteCalculator.calculateExposure(orderManager.getOpenOrders(), positionTracker.getAllInventories());
        
        // Cap total capital per slot to maxLossPerSlot * some factor (e.g., if maxLoss=10, cap total at 30)
        BigDecimal maxLossUsd = cfg.directionalMaxLossPerSlot();
        BigDecimal totalCapitalCap = maxLossUsd != null ? maxLossUsd.multiply(BigDecimal.valueOf(3)) : BigDecimal.valueOf(30);
        
        // Place UP order
        if (upShouldTrade) {
            BigDecimal baseShares = quoteCalculator.calculateShares(market, upBid, cfg, secondsToEnd, exposure);
            if (baseShares != null) {
                BigDecimal upShares = baseShares.multiply(BigDecimal.valueOf(upSizeMultiplier)).setScale(2, RoundingMode.DOWN);
                
                // Cap to not exceed total capital cap
                BigDecimal upCapital = upShares.multiply(upBid);
                if (upCapital.compareTo(totalCapitalCap) > 0) {
                    upShares = totalCapitalCap.divide(upBid, 2, RoundingMode.DOWN);
                }
                
                if (upShares.compareTo(BigDecimal.valueOf(0.01)) >= 0) {
                    log.info("DIRECTIONAL: {} UP @ {} x {} ({}x, {})", 
                            market.slug(), upBid, upShares, upSizeMultiplier, upIsDirectional ? "directional" : "hedge");
                    maybeQuoteToken(market, market.upTokenId(), Direction.UP, upBook, downBook, cfg, secondsToEnd, 0, upBid);
                }
            }
        }
        ...
        // Place DOWN order
        if (downShouldTrade) {
            BigDecimal baseShares = quoteCalculator.calculateShares(market, downBid, cfg, secondsToEnd, exposure);
            if (baseShares != null) {
                BigDecimal downShares = baseShares.multiply(BigDecimal.valueOf(downSizeMultiplier)).setScale(2, RoundingMode.DOWN);
                
                // Cap to not exceed total capital cap
                BigDecimal downCapital = downShares.multiply(downBid);
                if (downCapital.compareTo(totalCapitalCap) > 0) {
                    downShares = totalCapitalCap.divide(downBid, 2, RoundingMode.DOWN);
                }
                
                if (downShares.compareTo(BigDecimal.valueOf(0.01)) >= 0) {
                    log.info("DIRECTIONAL: {} DOWN @ {} x {} ({}x, {})", 
                            market.slug(), downBid, downShares, downSizeMultiplier, downIsDirectional ? "directional" : "hedge");
                    maybeQuoteToken(market, market.downTokenId(), Direction.DOWN, downBook, upBook, cfg, secondsToEnd, 0, downBid);
                }
            }
        }
    }
