...
def get_hft_stats():
    ...
    try:
        result = subprocess.check_output(['tail', '-500', HFT_LOG]).decode()
...
def get_opportunity_stats():
    """Parse old binary log for opportunities."""
    try:
        result = subprocess.check_output(['tail', '-1000', OLD_LOG]).decode()
        
        # Count arbitrage opportunities (combined < $0.98)
        opp_pattern = r'YES: \$([0-9.]+) \+ NO: \$([0-9.]+) = \$([0-9.]+)'
        matches = re.findall(opp_pattern, result)
        ...
        # Estimate ghost rate (typical 35%)
        ghost_rate = 0.35
        ...
        pass
    return {'total': 0, 'opportunities': 0, 'ghost_count': 0, 'executable': 0}
...
├ Arb detected: {opp['opportunities']}
├ Ghost rate: ~35%
└ Executable: {opp['executable']}

📈 Status:
HFT: ✅ Running (sub-µs)
Old: ✅ Running (tracking)
Disk: 19% ✅"""
