...
//! 
//! FIX (2026-04-01): Use clobTokenIds field (not tokens array)
//! Token IDs are stored as JSON string in clobTokenIds field
...
/// 
/// FIX: Use clobTokenIds field which contains token IDs as JSON string
pub async fn fetch_active_crypto_markets(
    ...
    // Search for btc-updown markets specifically
    let url = "https://gamma-api.polymarket.com/markets?search=btc-updown&limit=100&active=true&closed=false";
    ...
    for market in &markets {
        ...
        // FIX: Get token IDs from clobTokenIds field (JSON string)
        let clob_token_ids_str = market["clobTokenIds"].as_str().unwrap_or("[]");
        
        // Parse the JSON string to get token IDs
        let clob_token_ids: Vec<Value> = match serde_json::from_str(clob_token_ids_str) {
            Ok(v) => v,
            Err(_) => continue, // Skip if can't parse
        };
        
        if clob_token_ids.len() != 2 {
            continue; // Must have exactly 2 tokens
        }
