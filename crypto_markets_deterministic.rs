...
//! 
//! FIX (2026-04-01): Deterministic slug generation (NotebookLM recommendation)
//! Instead of searching/filtering 2000+ markets, we calculate exact slugs mathematically
//! This bypasses pagination, sorting, and zombie market issues
...
/// 
/// Returns slugs for current period and next period (for pre-subscription)
pub fn generate_target_slugs(asset: &str, interval_mins: u64) -> Vec<String> {
    ...
    // E.g., if now=100 and interval=15min(900s): current_period_end = 900
    let current_period_end = ((now / interval_secs) + 1) * interval_secs;
    
    // Next period for pre-subscription
...
/// 4. Return token IDs for markets that exist
